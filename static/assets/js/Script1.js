...
function generateQr(v) {
  if (inputValue.value) {
    ...
    // if (checkBox.checked || !switch_btn.checked) {
    if (!switch_btn.checked) {
      // if checkbox is checked
      if (fileSelector.value !== "") {
        // if user uploaded logo is available
        var imageBlob = new Blob([fileSelector.files[0]], {
          type: fileSelector.files[0].type,
        });
        ...
        if (properties.dotsOptions.color == "black") {
          // for black theme
          properties.image = "default_pic.png";
        } else {
          // for blue theme
          properties.image = "default_pic.png";
        }
    ...
    download[1].onclick = () =>
      qr.download({ name: "qr-code", extension: "jpg" });
  }
}
